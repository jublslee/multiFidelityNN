# This script only handles the data generation part of the example: 2D reaction-diffusion equations
# Code from the PDEBench repository:
#       https://github.com/pdebench
# Paper:
...
#----------------------------------------------------------------------------------------#
# RD simulator via RK4 and first order finite volume method
# Inputs:
#       Dc1: diffusivity constant for the first species
#       Dc2: diffusivity constant for the second species
#       kappa: reaction constant
#       time:  simulation ending time
#       t_dim: number of time steps
#       dimx: number of points in x-dir
#       dimy: number of points in x-dir
# Outputs:
#       use simulator obj to generate samples
def RD_simulator(  Dc1, Dc2, kappa, time, t_dim, dimx, dimy ):


    # required args for the simulator
    sim_obj = Simulator(                    Du   = Dc1,
                                            Dv   = Dc2,
                                            k    = kappa,
                                            t    = time,
                                            tdim = t_dim,
                                            ...
                                            ydim     = dimy,
                                            n = 1,
                                            seed = 1 # random seed for IC, it is fixed
                                            
                                            )

    return sim_obj.generate_sample()
#----------------------------------------------------------------------------------------#
...
#----------------------------------------------------------------------------------------#
# The reaction-diffusion equation solver class
# Code from:
#           https://github.com/pdebench/PDEBench/blob/main/pdebench/data_gen/src/sim_diff_react.py
# construct simulator obj
class Simulator:
    ...
    def generate_sample(self):
        ...
        # boost tolerance if needed
        #----------------------------------------------------------------------------------#
        prob = solve_ivp(self.rc_ode, (0, self.T), u0, t_eval=self.t, rtol=1e-4, atol=1e-6)
        #----------------------------------------------------------------------------------#
