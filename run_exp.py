...
# Define parameters to pass in NN
lf_list = [LF_Problem()]
...
# target values for each LF and HF problem
targets_L = LF_Problem().eval(data)
targets_H = HF_Problem().eval(data)
...
data = torch.linspace(0.0, 1.0, 100).unsqueeze(1)
hf = HF_Problem().eval(x)
lf = LF_Problem().eval(x)
